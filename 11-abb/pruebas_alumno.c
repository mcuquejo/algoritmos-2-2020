...
bool imprimir_un_item(const char* clave, void *dato, void *extra)
{
    // Sabemos que ‘extra’ es un entero, por tanto le podemos hacer un cast.
    int *contador = extra;
    printf("%d. clave: %s dato: %s\n", ++(*contador), clave, (char*) dato);

    return true; // seguir iterando
}

void imprimir_iter_interno(abb_t *arbol) {
    int num_items = 0;
    abb_in_order(arbol, imprimir_un_item, &num_items);
    printf("Tengo que comprar %d ítems\n", num_items);
}

void ejemplo_iterador_interno() {
    abb_t* abb = abb_crear(comparar_clave, NULL);
